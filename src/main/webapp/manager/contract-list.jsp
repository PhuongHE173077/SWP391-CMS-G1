<%-- Document : contract-list Created on : Dec 10, 2025, 10:44:22 AM Author : ADMIN --%>
                            ...
                            <form action="user-list" method="get">
                                ...
                                <div class="row mb-3 align-items-center bg-light p-2 rounded mx-0">
                                    <div class="col-md-6 d-flex align-items-center gap-3">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="sortBy" value="content"
                                                ${sortBy=='content' ? 'checked' : '' }>
                                            <label class="form-check-label">Name</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="sortBy"
                                                value="customerName" ${sortBy=='customerName' ? 'checked' : '' }>
                                    ...
                                    <div class="col-md-2">
                                        <select name="role" class="form-select">
                                            ...
                                            <c:forEach items="${lstSaleStaff}" var="s">
                                                <option value="${s.id}" ${createdByValue==s.id ? 'selected' : '' }>
                                    ...
                                    <div class="col-md-4">
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="search" class="form-control"
                                                placeholder="Search by Id, Content, Customer Name, Url Content, Created By..."
                                                value="${searchValue}">
                                        </div>
                                    </div>
                                    <div class="col-md-2 d-flex gap-2">
                                        <button type="submit" class="btn btn-primary w-100 fw-bold">Search</button>
                                        <a href="contract-list" class="btn btn-outline-secondary w-100"
                                            title="Reset Filter">
                                            Reset Filter
                                        </a>
                                    </div>
                                        ...
                                        <c:forEach items="${userList}" var="u">
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <c:if test="${u.gender}"><span
                                                            class="badge bg-light text-primary border">Male</span>
                                                    </c:if>
                                                    <c:if test="${!u.gender}"><span
                                                            class="badge bg-light text-danger border">Female</span>
                                                    </c:if>
                                                </td>
                                                <td class="text-center"><span
                                                        class="badge bg-info text-dark border">${u.roles.name}</span>
                                                </td>
                                                <td class="text-center">
                                                    <c:if test="${u.active}">
                                                ...
                                                <td class="text-center">
                                                    <div class="d-flex justify-content-center gap-2">
                                                        ...
                                                        <a href="edit?id=${u.id}"
                                                            class="btn btn-sm btn-outline-warning fw-bold text-dark"
                                                            style="min-width: 60px;">
                                                            Edit
                                                        </a>
                                ...
                                <nav aria-label="Page navigation">
                                    <ul class="pagination m-0">
                                        <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                            <a class="page-link"
                                                href="user-list?page=${currentPage - 1}&search=${searchValue}&role=${roleValue}&status=${statusValue}&gender=${genderValue}&sortBy=${sortBy}&sortOrder=${sortOrder}">Previous</a>
                                        </li>

                                        <c:forEach begin="1" end="${totalPages}" var="i">
                                            <li class="page-item ${i == currentPage ? 'active' : ''}">
                                                <a class="page-link"
                                                    href="user-list?page=${i}&search=${searchValue}&role=${roleValue}&status=${statusValue}&gender=${genderValue}&sortBy=${sortBy}&sortOrder=${sortOrder}">${i}</a>
                                            </li>
                                        </c:forEach>

                                        <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                            <a class="page-link"
                                                href="user-list?page=${currentPage + 1}&search=${searchValue}&role=${roleValue}&status=${statusValue}&gender=${genderValue}&sortBy=${sortBy}&sortOrder=${sortOrder}">Next</a>
                                        </li>
                                    </ul>
                                </nav>
                            </c:if>
